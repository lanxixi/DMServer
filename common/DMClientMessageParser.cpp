...
DMClientMessageHead DMClientMessageParser::parseHead(char * begin)
{
	DMClientMessageHead msg_head;
...
template <typename T>
void DMClientMessageParser::DMGetBitData(char *src,T *dsc,int bit_s,int bit_e)
{
	char *head_info = src;
...
int DMClientMessageParser::pack(DMClientMessage & mesg, char * buf)
{
